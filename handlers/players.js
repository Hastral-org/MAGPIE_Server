...
module.exports = (io, socket) => {
  ...
  if (!Number(playerID) || playerID === "undefined" || playerID === "null")
    return;
  socket.join(`player_${playerID}`);
  const player = MAGPIE_HIVE._get_player(playerID);
  if (!player)
    return MAGPIE_SYSTEM.log(
      `[PLAYER] unable to find [PLAYER-${playerID}]`,
      "console",
      false,
    );
  ...
  socket.on("player_action", (payload) => {
    MAGPIE_SYSTEM.log(
      `${ePrefix} Action received: ${payload.type}`,
      "console",
      false,
    );
  ...
  socket.on("request_sync", (payload) => {
    ...
    const player = MAGPIE_HIVE._get_player(playerID);
    if (!player)
      socket.emit("request_sync_error", {
        message: `unable to find [PLAYER-${playerID}]`,
      });
    player.status = true;
